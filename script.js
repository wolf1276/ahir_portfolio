/* ============================================
   PORTFOLIO — YAML-driven renderer
...
(function () {
  ...
  function renderAbout(data) {
    ...
    var links = (data.links || []).map(function (l) {
      ...
      return '<a href="' + esc(l.url) + '"' + target + '>' + icon + ' ' + esc(l.label) + '</a>';
    }).join('');
  ...
  function renderFooter(links) {
    ...
    var footerLinks = (links || []).map(function (l) {
      ...
      return '<a href="' + esc(l.url) + '"' + target + '>' + icon + ' ' + esc(label) + '</a>';
    }).join('');
  ...
  fetch('content.yaml')
    ...
    .then(function (text) {
      ...
      // Init scroll tracking after content is rendered
      initScrollTracking();
    })
    .catch(function (err) {
